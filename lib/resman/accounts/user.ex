defmodule Resman.Accounts.User do
  use Ecto.Schema
  import Ecto.Changeset

  @role_values ~w/USER ADMIN CAPTAIN/a
  ...
  defp validate_first_name(changeset) do
    ...
    |> validate_length(:first_name, min: 4, max: 30)
  end

  defp validate_last_name(changeset) do
    changeset
    |> validate_required([:last_name])
    |> validate_length(:last_name, min: 4, max: 30)
  end
  ...
  defp maybe_validate_unique_email(changeset, opts) do
    if Keyword.get(opts, :validate_email, true) do
      changeset
      |> unsafe_validate_unique([:email, :user_name], Resman.Repo)
  ...
  """
  def valid_password?(%Resman.Accounts.User{hashed_password: hashed_password}, password)
